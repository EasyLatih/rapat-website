...
function gigProviderServices(p){
  ...
  const standard=rows.map(x=>{const s=Array.isArray(x.gig_services)?x.gig_services[0]:x.gig_services;const c=Array.isArray(s?.gig_categories)?s.gig_categories[0]:s?.gig_categories;return [c?.name,s?.name].filter(Boolean).join(' · ')}).filter(Boolean).filter(x=>x!=='Lain-lain · Lain-lain');
  const custom=(Array.isArray(p.gig_provider_custom_services)?p.gig_provider_custom_services:[]).map(x=>`Lain-lain · ${x.name}`).filter(Boolean);
